...
- [ ] [observation] `vault:search_files` ne recurse PAS avec un glob simple `*X*` (cherche à la racine seulement) → 8 recherches à vide avant de comprendre qu'il faut préfixer `**/` (`**/*Wittouck*`). Réflexe : toujours préfixer les patterns de `vault:search_files` par `**/`. - 21-06-2026
- [ ] [observation] Pas d'outil de recherche par CONTENU ni de recherche sémantique/RAG exposé côté MCP vault : seulement recherche par nom de fichier (`search_files`) + lecture. Trouver une note par sujet quand on ignore son nom de fichier oblige à deviner le nom et multiplier les essais. Un `ctx_search`/grep sur le contenu manque à l'appel. - 21-06-2026
