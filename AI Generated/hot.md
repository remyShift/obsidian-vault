...
## Derniere activite
24-06-2026 - olis-lab : nouveau ticket "montant dynamique dans la top banner CMS" planifie (plan EN, 5 options pros/cons/faisabilite, **option non tranchee**) ; reco = token `{freeShippingThreshold}` injecte cote front via helper partage. Plus tot : bulk-add Products→Edit affine (Approche B) + `join` field evalue, et cadrage TASK-1115 (regeneration SKU).

## Projets actifs

### olis-lab
- Derniere session : 24-06-2026
- Etat : **Top banner montant dynamique (en planification)** : plan EN (`~/.claude/plans/dans-ma-top-banner-lexical-willow.md`), approuvé, **ticket ouvert option non choisie**. Banner = double source via flag `isDevAnnouncementBarEnabled` (CMS segments statiques vs fallback hardcodé qui contient déjà le message free-shipping dynamique). Payload sans built-in templating → injection au render front ; seuil = contexte client (`useFreeShippingThreshold` web / `selectedService.freeshipping` web_client). Reco Option 3 = token + helper pur partagé `@olis-lab/shared`, sans changement de schéma. **Bulk-add Products→Edit** : plan EN (`~/.claude/plans/en-suivant-le-template-rustling-toucan.md`), `Edits.products` (`hasMany`) existe déjà → pas de schéma, corps = Approche B (composant + endpoint custom) ; `join` field = companion découplé (YAGNI). **TASK-1115 (SKU)** cadré (pas de code) : PR auto-regen-on-brand-change revertée car SKU = clé BigBlue (changer un SKU live → out-of-stock) ; repo ne pousse jamais de SKU, pas de RBAC (Users auth nu) ; reco Option D (duplicate) + B (correction), RBAC différé. **Chantiers en attente** : `chore/remove-legacy-cra-checkout` (5 commits, NON pushé) + `feat/footer-global-cra-read` (PR ouverte). Pieges : `mise exec` ne prime pas sur node 21 système (forcer PATH), Node 20.19, lint-staged+prettier au pre-commit.
- Prochaine etape : banner — trancher l'option (si Option 3 : helper+test TDD, câbler les 2 hooks, admin description) ; bulk-add — décider l'implé B + répondre à Diego sur le join ; SKU — brief EN au meeting, trancher A vs B + RBAC avec Michele/Diego. En parallèle : push checkout branch + PR + e2e ; suivre retours PR footer ; rejouer seed stage/prod ; régénérer creds S3.
