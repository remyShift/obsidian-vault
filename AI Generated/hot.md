...
02-07-2026 - olis-lab : navbar CTA "Shop All" rendu editable via le global Payload `navbar` (groupe "Bottom CTA", label localise + path), branche dans web_client ET apps/web, **PR #1874 ouverte**. Pas de migration : seed manuel (cms_dev deja rempli). Piege decouvert : `.env.local` (Atlas cms_dev) prime sur `.env` pour `payload migrate`. PR #1870 (ProductActives) toujours en attente avec son bug limit.
