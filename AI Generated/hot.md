...
## Derniere activite
26-06-2026 - olis-lab : cart hook — reintroduction de #1801 (`computeCartSnapshot`, reverte par Diego) + 1 test de constat **implementee, PR #1859 ouverte** (9/9 tests d'integration verts en local). Decouverte : `reportError`→Sentry, donc chaque publication pre-backsync cree une erreur Sentry reelle → priorise le follow-up guard `beforeValidate`. Avant ca : plan valide (non implemente) de 4 ameliorations sur les globals Payload (`~/.claude/plans/j-aimerais-faire-le-plan-cuddly-breeze.md`).
