...
02-07-2026 - olis-lab : PR #1870 mergee mais migration executee (local+prod) avec le bug limit -> 10 actives traduits seulement (`payload.find` par defaut = 10 docs). Fix `pagination:false, limit:0` fait, PR ouverte par Remy ; re-run = delete record `payload-migrations` puis `pnpm migrate`. Navbar Bottom CTA editable (PR #1874 ouverte, web_client + apps/web). Piege du jour : `.env.local` prime sur `.env` pour `payload migrate`.
...
- Etat : **Migration ProductActives (#1870 MERGEE) incomplete en prod ET local** : executee avec `payload.find` sans `pagination:false, limit:0` -> limite par defaut 10 docs, seuls 10 actives traduits ; les autres = fallback en (rien d'ecrit, scalaire legacy intact). **Fix fait** (`fix/product-actives-migration-limit`, PR ouverte par Remy) ; re-run = Remy supprime le record `payload-migrations` (local+prod) puis `pnpm migrate` ; le re-run ecrase les 10 FR deja ecrits (accepte). Detection "fr manquant" seulement via `locale:'all'`/`fallbackLocale:'none'`. **Navbar Bottom CTA (PR #1874 ouverte, `refactor/add-bottom-cta-to-navbar-global`)** : groupe `cta` top-level dans le global `navbar` (label localise required + path required valide interne), consomme par web_client (`useNavbarQuery`/`HomepageNavbar`) et apps/web (`Navbar.tsx` + `NavbarMenu.tsx`, navigation `app:'legacy'`) ; `packages/ui` inchange ; guard `assertNavbar` fail-fast sans fallback -> seeder chaque env AVANT d'activer `dev_payload_navbar` (cms_dev seede, path a passer en `/products` ; stage/prod a faire) ; **pieges** : `.env.local` prime sur `.env` pour `payload migrate` (une migration test est partie sur cms_dev, nettoyee) ; `packages/shared` consomme via `dist/` -> rebuild avant typecheck. Review Diego traitee : `payload.find({locale:'en'})` remplace le driver natif — verdict par test int jetable : le read sert le scalaire legacy tel quel, mais un write `fr` en premier DROPPE l'original -> ordre EN-d'abord load-bearing. **BUG branche : find sans `pagination:false, limit:0` = 10 docs max, a fixer avant execution.** PR body : gotcha faux a reecrire ; 2 reponses EN Diego a poster. Fallback `true` : read `fr` vide sert `en` (aussi relations). **Chantiers en cours (inchanges)** : curated PLP (PR ouverte, bloquee par donnee = `cartProduct` manquant), cart #1859 (9/9 int verts, PR ouverte), navbar #1839 (fix posthog server flag, pas commite), PR #1853 SKU/EAN, plan globals `apps/cms` (T2->1->4->3). Securite : `NEXT_PUBLIC_FEATURE_FLAGS_SECURE_API_KEY` fuitee dans le bundle client, a renommer server-only.
...
- Prochaine etape : **merger la PR du fix limit**, puis Remy supprime le record `20260702_110143_localize_product_active_name` de `payload-migrations` (local + prod) et relance `pnpm migrate` (**verifier la base ciblee : `.env.local` prime**), lire le mapping EN->FR complet, relire les trads INCI dans l'admin. **PR #1874 navbar CTA : review + merge, path `/products` dans l'admin cms_dev, seeder stage/prod avant flag.** **curated PLP** = backfiller `cartProduct` + trancher drop-post-pagination + review/merge. cart #1859 = CI + review Diego + merge, puis guard `beforeValidate` legacyId (bruit Sentry/CI). Navbar #1839 = verif live Node 20 + commit/push + renommer cle API exposee (Amplify). PR #1853 = reponse Diego + verif admin + commit. globals = plan valide (T2->1->4->3). Parallele : slug 1850, banner / bulk-add / SKU meeting / RFC RBAC.
