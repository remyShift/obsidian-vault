...
## Derniere activite
24-06-2026 - olis-lab : **fix slug implemente** (Products + Subcategories) — le bouton "Generate" du CMS produisait des slugs malformes (`face--body-serum`) car `slugField` a 2 generateurs et TASK-1087 n'en corrigeait qu'un ; cable une seule fonction `slugify` resolver-aware en arg (bouton + save unifies), 29 tests verts, PR pas encore poussee. Plus tot (planning) : top banner montant dynamique (option non tranchee), bulk-add Products→Edit (Approche B), cadrage TASK-1115 (SKU).

## Projets actifs

### olis-lab
- Derniere session : 24-06-2026
- Etat : **Fix slug implémenté (Products + Subcategories), PR pas encore poussée** : bouton "Generate" du CMS produisait `face--body-serum` (double tiret, accents perdus) car `slugField` Payload a 2 générateurs indépendants — hook checkbox serveur (corrigé par l'override de TASK-1087) ET bouton client/preview via `slugifyHandler` qui lit `field.custom.slugify`, alimenté uniquement par l'arg `slugify` (jamais passé) → fallback slugify défaut Payload. Fix = override checkbox async (save) + arg `slugify` (bouton), tous deux via un resolver partagé (`computeProductSlug.ts` / `slugifySubcategory`) ; `useAsSlug` au lieu de `fieldToUse` (déprécié). NB : le hook `generateSlug` par défaut de Payload n'`await` PAS le slugify async → l'override reste obligatoire (premier jet sans override = tests d'intégration rouges, Promise dans `data.slug`). Typecheck/lint/int 20/20/unit 29/29 verts ; non vérifié en live ; migration subcategory non découplée (figer ou non à trancher). **Top banner montant dynamique (planif)** : plan EN approuvé, **option non choisie** ; reco Option 3 = token `{freeShippingThreshold}` + helper pur `@olis-lab/shared`, injection front, sans schéma ; seuil = contexte client. **Bulk-add Products→Edit (planif)** : `Edits.products` (`hasMany`) existe → pas de schéma, Approche B (composant + endpoint custom) ; `join` = companion découplé (YAGNI). **TASK-1115 (SKU)** cadré : PR auto-regen revertée car SKU = clé BigBlue (SKU live changé → out-of-stock) ; pas de RBAC ; reco Option D (duplicate) + B (correction). **Chantiers en attente** : `chore/remove-legacy-cra-checkout` (5 commits, NON pushé) + `feat/footer-global-cra-read` (PR ouverte). Pieges : `mise exec` ne prime pas sur node 21 système (forcer PATH), Node 20.19, lint-staged+prettier au pre-commit.
- Prochaine etape : slug — pousser/ouvrir la PR + vérifier le bouton Generate en live + décider figer la migration ; banner — trancher l'option (si Option 3 : helper+test TDD, câbler les 2 hooks, admin description) ; bulk-add — décider l'implé B + répondre à Diego sur le join ; SKU — brief EN au meeting, trancher A vs B + RBAC. En parallèle : push checkout branch + PR + e2e ; suivre retours PR footer ; rejouer seed stage/prod ; régénérer creds S3.
