...
24-06-2026 — **Fix slug implémenté** (Products + Subcategories) : le bouton "Generate" du CMS produisait des slugs malformés (`face--body-serum`), car `slugField` a 2 générateurs et TASK-1087 n'en corrigeait qu'un. Câblé une seule fonction `slugify` resolver-aware en arg → bouton + save unifiés. Typecheck/lint/29 tests verts, PR pas encore poussée. (Matin : planning banner + bulk-add + SKU.)

## État du projet
- **Slug fix (implémenté, à pousser)** : `computeProductSlug.ts` (`composeProductSlug` pur + `slugifyProduct` async brand-aware) câblé via `slugField({ useAsSlug, slugify })`, override `generateBrandTitleSlug` supprimé. Idem Subcategories (`slugifySubcategory`, slugify dupliqué → `@/lib/slugify`). Message PR EN prêt. Pas vérifié en live, pas commité.
...
## Décisions actives
- Slug : approche unifiée (1 fonction `slugify` en arg, override supprimé). Migration subcategory NON découplée (slugify corrigé sur env neuf) — figer ou non = à trancher. Test pré-existant corrigé (`vitamin-c-e`), test produit supprimé par Rémy.
