...
- Technique efficace pour développer des fonctionnalités complexe et permets de réduire les peurs et frustrations de part le cadre qu’il apporte

## Principes et étapes :

- TDD = écrire le test avant le code —> fixer l’objectif et définir le comportement attendu
	- Permets d’écrire le moins de code pour satisfaire un objectif 
		- Moins de code = + de maintienbilité
...
⚠️ Il faut être le plus rigoureux possible lors de ses étapes et bien avancer pas à pas.

**NB :** cf [[Anatomie d'un test]]

Pour résumer l’objectif du TDD est de définir un objectif / besoin sous la forme d’un test qu’on veut faire passer au vert avec le minimum de code possible avant de passer a essayer de rendre notre code plus lisible.